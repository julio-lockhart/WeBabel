...
const RoomMessageList = ({ rooms = [], user, messages, room, actions }) => {
   ...
   rooms.forEach((room) => {
      ...
      if (isFinite(oldestMessageIdReceived)) {
         ...
         const senderName = (senderId === user.id) ? 'You' : messages[room.id][oldestMessageIdReceived].userStore.store.store[senderId].name;
         const latestMessage = messages[room.id][oldestMessageIdReceived].text;

         const currentUserAvatar = user.avatarURL ? user.avatarURL : "https://image.flaticon.com/icons/svg/149/149071.svg";
         const senderAvatar = messages[room.id][oldestMessageIdReceived].userStore.store.store[senderId].avatarURL;
         const avatar = (senderId === user.id) ? currentUserAvatar : (senderAvatar ? senderAvatar : "https://image.flaticon.com/icons/svg/149/149071.svg");
